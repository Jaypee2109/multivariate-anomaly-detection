...
class OldRotaryEncoderLayer(nn.Module):
    ...
    def forward(
        self, src, src_mask=None, **kwargs
    ):  # existing logic, no need to use is_causal
        seq_len, _, _ = src.size()
        cos, sin = self.rotary_emb.get_embedding(
            seq_len, src.device
        )  # flatten seq & batch
        src2d = src.reshape(-1, src.size(-1))  # [seq_len * batch, d_model]
        qkv = F.linear(
            src2d, self.self_attn.in_proj_weight, self.self_attn.in_proj_bias
        )
        ...
        k = self.rotary_emb.apply_rotary(
            k, cos, sin
