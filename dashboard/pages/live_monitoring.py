...
def update_dataset_options(category: str) -> tuple[list, str | None]:
...
def tick(
    ...
    if new_idx <= 0:
        return (
            _empty_fig("Press Play to start"),
            NO,
            {"index": 0},
            NO,
            [],
    ...
    cache_hit = cache.get("models_key") == models_key and cache.get("valid_to", 0) >= total
    ...
    if not cache_hit:
        ...
        if result is None:
            ...
            return (
                ...
                new_state,
                NO,
                [html.Span("API offline", className="text-muted-light")],
                f"{new_idx:,} / {total:,}",
                "",
                _api_badge(False),
                finished,
...
def _build_progressive_summary(
    ...
    models_data = cache.get("chart_data", {}).get("models", {})
    ...
    for slug, mdata in models_data.items():
        ...
        summary[slug] = {
            "display_name": slug,
            "anomaly_count": count,
            "anomaly_ratio": count / visible_to,
        }
