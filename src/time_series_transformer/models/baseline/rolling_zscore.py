...
class RollingZScoreAnomalyDetector(BaseAnomalyDetector):
    ...
    def __init__(self, window: int = 12, z_thresh: float = 3.0, min_periods: int | None = None):
        self.window = window
    ...
    def _compute_zscores(self, y: pd.Series) -> pd.Series:
        ...
        rolling_mean = combined.rolling(self.window, min_periods=self.min_periods).mean()
